...
const VARS = [
  ...
  { name: 'STAFF_DISCORD_ROLE_IDS', group: 'Discord', doc: 'Staff Discord roles and the servers each one covers: comma-separated role ids, each optionally followed by a colon and its servers joined with +, for example 111:*,222:eu1,333:na1+na2. A bare role id covers every server. When the priority queue behind a game.admins entry the shop added lapses, a player holding a role that covers that server stays listed; anyone else is removed. Blank = the ticket bot\'s staff roles in code: the admin roles cover every server, each Gamemaster role only its own.', example: '' },
